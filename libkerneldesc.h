...
static MatrixXf EvalKernelExp_Matlab( const MatrixXf& data1_f, const MatrixXf& data2_f, float kparam )
{
  MatrixXd data1 = data1_f.cast<double>();
  MatrixXd data2 = data2_f.cast<double>();
...
static void GKDESDense(MatrixXf& feaArr, MatrixXf& feaMag, MatrixXf& fgrid_y, MatrixXf& fgrid_x, IplImage* im, matvarplus_t* kdes_params, int grid_space, int patch_size, float low_contrast) {
	...
	/*
	CvScalar s;
	for( int i = 0; i < imX->height; i++ ){
	  for( int j = 0; j < imX->width; j++ ){
	    s = cvGet2D( imX, i, j );
	    std::cout << s.val[0] << " ";
	  }
		std::cout << std::endl;
...
static void CKSVDEMK(MatrixXf& imfea, const MatrixXf& feaArr, const MatrixXf& feaMag, const MatrixXf& fgrid_y, const MatrixXf& fgrid_x, const int img_h, const int img_w, MatrixXf& words, MatrixXf& G, MatrixXf& pyramid, const float kparam)
{
	...
	MatrixXf pgrid = pyramid.array().pow(2);//pyramid[] = 1, 2, 3
	...
	//MatrixXf kz = EvalKernelExp_Matlab( (feaArr), words, kparam );
	
	for(int s=0; s<pyramid.size(); s++)//At matlab, length(emk_params.pyramid) at line 20
	{
		...
		for(int t=1; t<=pgrid(s); t++)//At matlab, pgrid(s)
		{
...
public:
	KernelDescManager(string model_name,string model_file,string model_var,string param_file, 
			  unsigned int MODEL_TYPE=0, int MAX_IMAGE_SIZE=0);
	~KernelDescManager();
