			...
			if (img_src != NULL)
			{
				if (run_grab_cut) {
				  ...
				  cv::grabCut( image, mask, rect, bgdModel, fgdModel, (1), cv::GC_INIT_WITH_RECT);
					 cv::Mat submask=mask( cv::Range(ycenter-centersize, ycenter+centersize),cv::Range(xcenter-centersize,xcenter+centersize) );
					 submask=cv::GC_FGD;
				  ...
				  image.copyTo(foreground,mask); // bg pixels not copied
					...
					cvRectangle( &CvMat(foreground), cv::Point(xcenter-centersize,ycenter-centersize), cv::Point(xcenter+centersize,ycenter+centersize), cv::Scalar(0,0,255) );
					cvRectangle( &CvMat(foreground), cv::Point(startx,starty), cv::Point(endx,endy), cv::Scalar(0,0,255) );
...
int main(int argc, char* argv[]) {
	...
	{
		cout << "Enter '1' for camera demo (full image), '2' for camera demo (w/ GrabCut) or '3' for demo with image dataset: ";
		cin >> opt;
		cout << "Opt: " << opt << endl;
		if (opt==1 || opt==2)
		{
